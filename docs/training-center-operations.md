...
| `TRAINING_AI_MODEL` | No | Usa el valor por defecto (`google/gemini-3.6-flash`, definido una única vez en `src/lib/ai-model.ts`). Vacía o con solo espacios cuenta como ausente, así que también cae al defecto. Dos avisos operativos sobre el defecto: cuesta más que el `google/gemini-2.5-flash` anterior (1,50 frente a 0,30 USD por millón de tokens de entrada; 7,50 frente a 2,50 de salida) y los modelos 3.x **razonan por defecto**, lo que añade tokens de salida y **latencia**. Fijar esta variable revierte a cualquier otro modelo sin desplegar; `google/gemini-3.5-flash-lite` mantiene el precio del modelo anterior. |
| `TRAINING_ANALYSIS_CALL_TIMEOUT_MS` | No | Usa el valor por defecto (20.000 ms de tope para **una** llamada de análisis de documento). Solo acepta un entero positivo; cualquier otro valor —vacío, `0`, negativo, decimal, texto— cae al defecto. Súbelo si los resúmenes empiezan a salir marcados como parciales de forma sistemática: es el síntoma de que las llamadas se cortan por latencia, no de que fallen. El techo real lo sigue poniendo el presupuesto total del análisis (`TRAINING_ANALYSIS_TIME_BUDGET_MS`, 35 s), con el que se combina por `Math.min`, así que subirlo por encima de eso no consigue nada. |
