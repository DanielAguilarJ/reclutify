...
import { sanitizeText, generateUsername } from '@/lib/utils';
...
export async function updateProfile(
  ...
  if (payload.certifications !== undefined) {
    updates.certifications = payload.certifications.slice(0, 20);
  }
  if (payload.languages !== undefined) {
    updates.languages = payload.languages.slice(0, 15);
  }
...
export async function incrementProfileViews(profileId: string): Promise<void> {
  try {
    ...
    // Non-critical — view count increment is best-effort
  }
}
...
// ─── Public: Get user's recent posts for profile activity ───

export async function getUserRecentPosts(userId: string, limit = 5): Promise<{
  id: string;
  content: string;
  likes_count: number;
  ...
  try {
    ...
    return (data || []) as { id: string; content: string; likes_count: number; comments_count: number; created_at: string; }[];
  } catch {
    return [];
  }
}
